...
public class Helpers {
	...
	public static String validatePatientId(Scanner scanner) {
	    final Pattern PATIENT_ID_PATTERN = Pattern.compile("^(GP|EP)\\d{4,}$");

	    while (true) {
	        System.out.print("Enter Patient ID (GP1000 or EP9000 format): ");
	        String input = scanner.nextLine().trim().toUpperCase();
	        ...
	        if (PATIENT_ID_PATTERN.matcher(input).matches()) {
	            return input;
	        }

	        System.out.println("Invalid format. Use GP/EP prefix followed by digits (e.g. GP1000, EP9000).");
	    }
	}
}